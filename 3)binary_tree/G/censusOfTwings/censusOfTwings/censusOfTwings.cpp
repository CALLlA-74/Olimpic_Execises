...
void CensusOfTwings(_node* root) {
    if (root->left != NULL) CensusOfTwings(root->left);
    if ((root->left != NULL && root->right == NULL) || (root->left == NULL && root->right != NULL)) cout << root->key << endl;
    if (root->right != NULL) CensusOfTwings(root->right);
}
